...
const WhatsAppInquiryButton: React.FC<WhatsAppInquiryButtonProps> = ({ 
  ...
  const handleWhatsAppInquiry = () => {
    ...
    const getBaseUrl = () => {
      ...
      if (import.meta.env.PROD) {
        return 'https://technology-mart.vercel.app';
      }
      // In development, use the current origin
      return window.location.origin;
    };
  
    // Create product URL for the current product
    const productUrl = `${getBaseUrl()}/?product=${product.id}`;
  ...
  const isDisabled = !socialMediaLinks.whatsapp || footerLoading;
