...
const Cart: React.FC<CartProps> = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50">
      <div ref={cartRef} className="fixed right-0 top-0 h-full w-full max-w-md bg-slate-900 text-white shadow-xl">
        <div className="flex flex-col h-full">
          {/* Header */}
          <div className="relative py-8 px-6 border-b border-slate-700">
            <div className="absolute inset-0 bg-gradient-to-r from-blue-600/20 to-purple-600/20" />
            <div className="relative flex flex-col items-center">
              <h2 className="text-2xl font-bold text-center mb-2">Shopping Cart</h2>
              <p className="text-slate-400 text-sm">{state.items.length} items</p>
            </div>
            <button
              onClick={closeCart}
          ...
          {/* Footer */}
          {state.items.length > 0 && (
            <div className="border-t border-slate-700 p-6 space-y-4 bg-slate-800/30 backdrop-blur-sm">
              ...
              <div className="space-y-3">
                ...
                <button 
                  onClick={closeCart}
                  className="w-full border border-slate-600 hover:bg-slate-700/50 text-slate-300 py-3 px-4 rounded-lg font-semibold transition-colors"
                >
                  Continue Shopping
                </button>
              </div>
